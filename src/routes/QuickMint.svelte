<script>
    ...
    import { erc721Simple, erc721Merkle } from "$lib/contract.config.js";    
    ...
    // Constants
    const mintContract = "0xcdcc5574e2dA9190BE3d2f20b7EAAAecd708bf07";
    ...
    async function mintNFT() {
        ...
        try {
            ...
            const collectionContract = new ethers.Contract(mintContract, erc721Merkle, signer);
            const mintTx = await collectionContract.mint(
                address,
                BigInt(drop.tokenId),
                drop.tokenURI,
                false, // public mint, not whitelist
                proof,
                {
            ...
            await mintTx.wait();
            const id = drop.tokenId;

            // Add to wallet storage
            addNFTToWallet(address, { collectionAddress: mintContract, id });
            ...
            await generateNFTSkill(mintContract, attributes, id, address);
        ...
        <div class="z-10 text-white relative flex flex-col justify-center items-center self-center min-h-[40vw] max-h-[40vw] w-[40vw]">
            ...
            <div class="flex flex-col w-full items-center border-t mt-[2vw] gap-[0.1vw]">
                <span class="text-[0.9vw] mt-[1vw] font-normal text-gray-200">
                    Already have a NFT on U2U chain?
                </span>
